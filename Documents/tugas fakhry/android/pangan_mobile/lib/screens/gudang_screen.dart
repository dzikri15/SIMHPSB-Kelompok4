...
class _GudangScreenState extends State<GudangScreen> {
  ...
  Widget _buildAlertSection() {
    return FutureBuilder<List<StokModel>>(
      key: ValueKey('alerts_$_monitoringKey'),
      future: _stokService.getMonitoring(),
      builder: (_, snap) {
        if (!snap.hasData) return const SizedBox.shrink();
        final alerts = snap.data!
            .where((s) =>
                s.isLow && !dismissedAlerts.contains(s.id.toString()))
            .toList();
        if (alerts.isEmpty) return const SizedBox.shrink();
        return Column(
          children: [
            ...alerts.map((s) => StockAlertNotification(
                  namaKomoditas:
                      s.gudang?.namaGudang ?? 'Gudang ${s.gudangId}',
                  jumlahStok: s.jumlahStok,
                  status: s.monitoringStatus ?? 'rendah',
                  onDismiss: () => setState(
                      () => dismissedAlerts.add(s.id.toString())),
                )),
      ...
      children: [
        ...
        Row(children: [
          ...
          Expanded(
            ...
            items: const ['Beras', 'Gabah', 'Jagung', 'Singkong'],
            onChanged: (v) {
              _filterKomoditas = v;
              _applyFilter();
            },
